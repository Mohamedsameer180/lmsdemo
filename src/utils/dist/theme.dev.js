...
exports.resetTheme = exports.loadStoredTheme = exports.applyTheme = void 0;
 ...
 */
var applyTheme = function applyTheme(branding) {
  if (!branding) return;
  var root = document.documentElement; // Set CSS custom properties (variables)
  ...
  root.style.setProperty('--font-family', branding.fontFamily || 'Roboto, sans-serif'); // Persist theme in localStorage so it survives page reloads
...
var loadStoredTheme = function loadStoredTheme() {
  if (typeof window === 'undefined') return; // Skip on server-side

  try {
    var storedTheme = localStorage.getItem('lms_theme');
    ...
    console.error('Failed to load stored theme:', error); // Fallback to defaults on error
